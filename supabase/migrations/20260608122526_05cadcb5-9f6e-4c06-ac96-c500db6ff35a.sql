...
BEGIN
  ...
  first_win AS (SELECT user_id FROM user_stats WHERE correct_count >= 1),
  centurion AS (SELECT user_id FROM user_stats WHERE total_pts >= 100),
  perfect_pundit AS (SELECT user_id FROM user_stats WHERE exact_count >= 5),
  veteran AS (SELECT user_id FROM user_stats WHERE played_count >= 10),
  oracle AS (SELECT user_id FROM user_stats WHERE exact_count >= 10),
  daily_champ AS (SELECT DISTINCT user_id FROM public.reward_winners WHERE period_type='daily' AND rank=1),
  weekly_champ AS (SELECT DISTINCT user_id FROM public.reward_winners WHERE period_type='weekly' AND rank=1),
  monthly_champ AS (SELECT DISTINCT user_id FROM public.reward_winners WHERE period_type='monthly' AND rank=1),
  all_badges AS (
